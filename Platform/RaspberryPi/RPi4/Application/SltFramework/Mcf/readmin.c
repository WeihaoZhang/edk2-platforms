...
{                                       
    ...
    if(( in = fopen( net->inputfile, "r")) == NULL )
        return -1;
    Print(L"[SLT] %a:(%d) fopen ok............\n", __FUNCTION__, __LINE__);

    fgets( instring, 200, in );
    Print(L"[SLT] %a:(%d) fgets ok............\n", __FUNCTION__, __LINE__);
    
    char instring2[201];
    fgets( instring2, 100, in );
    Print(L"[SLT] %a:(%d) fgets ok............\n", __FUNCTION__, __LINE__);
    if( sscanf( instring, "%ld %ld", &t, &h ) != 2 )
        return -1;
    
    Print(L"[SLT] %a:(%d) sscanf ok,%ld,%ld............\n", __FUNCTION__, __LINE__,t,h);
    net->n_trips = t;
    net->m_org = h;
    net->n = (t+t+1); 
    net->m = (t+t+t+h);
...
*/
    ...
    Print(L"[SLT] %a:(%d) calloc ok,%ld,%ld............\n", __FUNCTION__, __LINE__,net->n + 1,net->max_m);
    Print(L"[SLT] %a:(%d) calloc ok,%x............\n", __FUNCTION__, __LINE__,net->nodes);
    Print(L"[SLT] %a:(%d) calloc ok,%x............\n", __FUNCTION__, __LINE__,net->arcs);
    Print(L"[SLT] %a:(%d) calloc ok,%x............\n", __FUNCTION__, __LINE__,net->dummy_arcs);
    ...
    {
        Print(L"[SLT] %a:(%d) enter loop ok,%d............\n", __FUNCTION__, __LINE__,i);
        fgets( instring, 200, in );
        Print(L"[SLT] %a:(%d) fgets ok,%d\n", __FUNCTION__, __LINE__,i);
        if( sscanf( instring, "%ld %ld", &t, &h ) != 2 || t > h )
            return -1;
        Print(L"[SLT] %a:(%d) sscanf ok,%d\n", __FUNCTION__, __LINE__,i);
