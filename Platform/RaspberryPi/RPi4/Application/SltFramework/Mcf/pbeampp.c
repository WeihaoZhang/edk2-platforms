...
static BASKET basket[BDATA +K+1];
static BASKET *perm[BDATA +K+1];
...
#endif
{
    ...
    {
        for( i = 2, next = 0; i <= BDATA && i <= basket_size; i++ )
        {
            arc = perm[i]->a;
...
NEXT:
    ...
    if( basket_size < BDATA && group_pos != old_group_pos )
        goto NEXT;
